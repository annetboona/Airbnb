...
const handleBookingError = (res: Response, error: any, operation: string) => {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === "P2002") return res.status(409).json({ message: "A booking with these details already exists." });
        if (error.code === "P2025") return res.status(404).json({ message: "Booking not found." });
        if (error.code === "P2003") return res.status(400).json({ message: "Invalid foreign key: Check your guestId or listingId." });
    }
...
export const getBookingById = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (role === "ADMIN") return res.json(booking);
        if (booking.guestId === uid) return res.json(booking);
        if (role === "HOST" && booking.listing.hostId === uid) return res.json(booking);
...
export const createBooking = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const parsedGuests = parseInt(guests as string);
