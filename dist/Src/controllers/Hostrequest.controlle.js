...
export const getMyHostRequest = async (req, res) => {
    try {
...
export const approveHostRequest = async (req, res) => {
    try {
        ...
        sendEmail(hostRequest.user.email, "🎉 Your Host Request Has Been Approved!", `<p>Hi ${hostRequest.user.name},</p>
       <p>Great news! Your request to become a host has been <strong>approved</strong>.</p>
       <p>You can now list your properties on the platform. Welcome to hosting!</p>`).catch((err) => console.error("Approval email failed:", err.message));
...
export const rejectHostRequest = async (req, res) => {
    try {
        ...
        // Notify user by email (non-blocking)
        sendEmail(hostRequest.user.email, "Your Host Request Update", `<p>Hi ${hostRequest.user.name},</p>
       <p>Unfortunately your host request has been <strong>declined</strong>.</p>
       ${adminNote ? `<p>Reason: ${adminNote}</p>` : ""}
       <p>You may resubmit with more details if you believe this was a mistake.</p>`).catch((err) => console.error("Rejection email failed:", err.message));
...
//# sourceMappingURL=Hostrequest.controlle.js.map